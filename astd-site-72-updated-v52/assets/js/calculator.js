...
function getBuild(){
  ...
  const activeRow = upgPick >= 0 && upgRows ? upgRows[upgPick] : null;
  const autoDps = abilityDpsOf(nm, fDmg, activeRow);
  return {baseDmg, baseSpa, baseRng, cost, fDmg, fSpa:baseSpa, fRng, ab, mDps, autoDps, activeRow,
          dps: unitTotalDps(nm, dmg1, baseSpa, copies, activeRow) + mDps + autoDps,   /* สถานะที่ไม่ซ้อนจะไม่คูณตามจำนวนตัว */
          baseDps: unitTotalDps(nm, baseDmg, baseSpa, 1) + abilityDpsOf(nm, baseDmg, null)};
}
...
function calc(){
  ...
  const ar = $("outAbilityRow");
  if(ar){
    ar.style.display = b.autoDps > 0 ? "" : "none";
    if(b.autoDps > 0){
      $("outAbilityLabel").textContent = abilityDpsLabel(selIdx >= 0 ? UNITS[selIdx][0] : "", b.activeRow);
      $("outAbility").textContent = fmt(b.autoDps);
    }
  }
  paintUpgradeNumbers();
  const orb = orbPick();
...
function renderUpgrades(name){
  ...
  table.innerHTML = upgRows.map((r,i)=>{
    ...
    const stats = r.farm != null ? [
      [lang === "en" ? "Cash per wave" : "เงินต่อเวฟ", fmtFull(r.farm)],
      [lang === "en" ? "Stored" : "เงินสะสม", fmtFull(r.stored || 0)],
    ] : [
    ...
    if(r.flat) stats.push([
      lang === "en" ? "Added damage (no Leader/Orb)" : "ดาเมจเพิ่ม (ไม่รับ Leader/Orb)",
      fmtFull(r.flat)
    ]);
    ...
    if(r.bleed && r.spa){
      const bBase = bleedDps(r.dmg, name, r), bFlat = bleedDps(fl, name, r);
    ...
    if(r.abilityDps && r.spa){
      const ratio = r.abilityDps.mult / r.abilityDps.cd;
      const xBase = r.dmg * ratio, xFlat = fl * ratio;
      stats.push([`${r.abilityDps.label || "Ability"} DPS`, fmt(xBase+xFlat), "xdps", xBase, xFlat]);
      stats.push([T.totalDps, fmt(tot/r.spa+xBase+xFlat), "xtot", r.dmg/r.spa+xBase, fl/r.spa+xFlat]);
    }
    ...
    return `<div class="upg-card" data-i="${i}"${r.flatSkip ? ` data-flatskip="${r.flatSkip.join(",")}"` : ""}${
      ...
      <div class="upg-cum">${T.upgCum} ${fmtFull(r.farm != null && r.stored != null ? r.stored : upgradeCumCost(upgRows, i))}</div>
    </div>`;
  }).join("");
...
function paintUpgradeNumbers(){
  ...
  document.querySelectorAll("#upgTable .upg-card b[data-k]").forEach(el=>{
    ...
    if(el.dataset.k === "bdps" || el.dataset.k === "btot"){
      const card = el.closest(".upg-card");
      const row = upgRows && upgRows[+card.dataset.i];
      const name = selIdx >= 0 ? UNITS[selIdx][0] : "";
      const spec = bleedSpec(name, row || null);
      const bEl = card.querySelector('b[data-k="bdps"]');
      const bB = parseFloat(bEl.dataset.base) || 0, bF = parseFloat(bEl.dataset.flat) || 0;
      const bleed = (bB * mA * mB + bF * mB) * (spec && spec.scaleCopies === false ? 1 : copies);
      if(el.dataset.k === "bdps"){
        el.textContent = fmt(bleed);
      } else {
        const totalB = parseFloat(el.dataset.base) || 0, totalF = parseFloat(el.dataset.flat) || 0;
        const normal = ((totalB - bB) * mA * mB + (totalF - bF) * mB) * copies;
        el.textContent = fmt(normal + bleed);
      }
      el.classList.toggle("buffed", m !== 1);
      return;
    }
...
function applyUpgrade(i){
  ...
  $("inCost").value = r.farm != null && r.stored != null ? r.stored : upgradeCumCost(upgRows, i);
...
document.addEventListener("DOMContentLoaded", ()=>{
  ...
  const stageIdx = parseInt(p.get("stage"));
  ...
  if(Number.isInteger(stageIdx) && upgRows && upgRows[stageIdx]) applyUpgrade(stageIdx);
});
